...
Performance budget: cold launch → window **< 250 ms** (measured ~200 ms; content ~360 ms), warm open into the resident instance **< 50 ms** (measured ~25 ms), in-app navigation to another document **< 50 ms**, 1 MB file parse+render **< 400 ms** (typical prose ~150 ms measured).

The app is **resident**: `HideWindowOnClose` keeps the process alive when the window is closed (Cmd+Q quits), so subsequent document opens skip process spawn and WebKit init entirely — LaunchServices delivers the file to the running instance and `openPath` re-shows the window (~25 ms). An optional login item (`scripts/prewarm.sh`, launches the app with `--hidden`) makes the first open of a session warm too: `--hidden` boots everything but keeps the window gated (`hiddenUntilOpen`) until the first open. Syntax highlighting is capped: any single code fence larger than 50 KB renders as a plain escaped `<pre><code>` block (no chroma) — chroma tokenization costs seconds per megabyte, so the cap keeps pathological code-heavy documents bounded.
